\section{CrashSimulator Approach }
\label{sec:approach}
    ...
    \preston{Much like the effort involved in constructing a unit test that
      checks for the presence of correct behavior in a piece of code, this
      process involves some amount of user skill and effort initially with the
      idea that this effort will be paid back as its products are used
      repeatedly in over time.  Consider a Linux application that depends on a
      ...
      \preston{With this information in mind, a developer can construct the simple
      ...
      makes {\tt open ()} calls.  In this way, the mutation can allow
   ...
   In many situations
    ...
    %% Anomaly Examples subsection removed on recommendation from Lois
