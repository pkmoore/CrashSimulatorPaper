...
environmental differences greatly exacerbates the problems in ensuring
...
\thomas{I think the following paragraph should give a short high-level
  overview of CrashSimulator's approach, specifically, the idea of
  system call replay, mutation, and checking. The summary of the
  contributions refers to some of these ideas but we never actually
  explain any of them in the intro.}
...
detected by CrashSimulator in other applications.  \thomas{What does
  'This' refer to?} This merely requires the 
environmental bug to be added to CrashSimulator \thomas{What does it
  mean for an environmental bug to be added to CrashSimulator?}. Once this is done, any application can
...
%software against an environment is an important step forward.  
\cappos{Please check}
There
...
\item \thomas{The summary of the evaluation should be expanded and
    beefed up (it should be at least as detailed as in the
    abstract). Also, perhaps move this bullet point into a separate
    paragraph after the enumeration for emphasis.}{It presents an implementation of
    CrashSimulator that can implement the discussed approach to
    successfully find bugs, both known and unknown.  Using this
    prototype on popular applications, such as {\tt wc}, {\tt ftp},
