...
LOC)\footnote{ Line count numbers are according to SLOCCOUNT\cappos{cite}}.  
For a test
environment, we chose a virtual install of Ubuntu Linux 15.04 with a 32 bit 
kernel on i686 hardware.
We ran the tests on a core i7 system with 8GB of RAM \cappos{add Ghz}. Our
...
\cappos{Term?} our 
implementation of CrashSimulator can
...
on popular projects as is indicated by Debian's Popularity Contest\cappos{add
cite}.
...
or disk and may crash the system \cappos{cite apt / yum endless data attacks}.
...
emulate the application being provided a file type that is unexpected.  For 
example, the 
{\tt sed} application, which modifies the contents of a text file according 
to a provided command string, could be provided something else such as a 
symbolic link, a directory, or a character device instead.  CrashSimulator
accomplishes this by identifying the calls to {\tt stat()}, {\tt fstat()}, or
{\tt lstat()} that an application makes to examine the file 
and then changes to trace to simulate that the regular file
{\tt sed} is expecting is actually one of the
...
not being handled correctly. \cappos{Is this true in all cases?}
...
space\cappos{Please label the bugs w/ the impact.} 
\cappos{cite apt / yum bugs}  
...
We use CrashSimulator to analyze 10 network applications and libraries
...
applications \cappos{include elapsed time needed to do a basic
session.} were vulnerable, with timeouts taking hours or
...
lived jobs consuming resources and causing deterimental system behavior.
 \cappos{cite SlowLoris}  Similar attacks can be used to indefinitely 
delay security updates to clients, leaving them vulnerable to compromise
\cappos{cite slow retrieval for Yum, Apt, etc.}.
...
frequently get wrong \cappos{cite bug reports related to mv}.  
%To come up with the
...
An application must make an effort to ensure that the file being copied is not 
...
performed, it often results in corruption of the file. \cappos{ Cite Python
bug 15100} 
...
executed without explicit user action. \cappos{cite} 
...
\cappos{I think these likely all get removed...  I don't understand the purpose}
