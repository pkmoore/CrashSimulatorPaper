...
\begin{enumerate}
   \item{Do environmental bugs exist in widely used
       applications? (Subsection~\ref{sec-env-bugs})}
...
\hline
        {\tt wc}            & Error: Is a Directory     & hangs                 & No output, slowly process file  & Hangs\\
        {\tt install}       & Error: Omitting Directory & Fills disk            & No output, copies file          & Hangs\\
        {\tt fmt}           & No output                 & hangs, garbage output & Processes file, garbage output  & Hangs\\
        {\tt od}            & Error: read error         & hangs, garbage output & No output                       & Hangs\\
        {\tt ptx}           & Error: Is a Directory     & hangs, fills disk     & Processes file, garbage output  & Hangs\\
        {\tt comm}          & Error: Is a Directory     & hangs, garbage output & Processes file, garbage output  & Hangs\\
        {\tt pr}            & Error: Is a Directory     & hangs, garbage output & Processes file, garbage output  & Hangs\\
\hline
    \bottomrule{}
    \end{tabular}
...
The first possibility is that CrashSimulator
asserts that the application will fail and the application fails in practice.
this is the case of the install and a character device result above.  CrashSimulator
predicted failure and the application ended up filling the disk of the machine
it was run on.  A second possibility is that CrashSimulator detects
that the application
may respond correctly in
practice.  This is the case of wc when tested with a directory.

The other two cases occur when there is disagreement. The first is where
CrashSimulator asserts that the application has made an effort to respond but in
reality the application fails in some way.  This is the case of {\tt ptx}.
CrashSimulator reported that it had made an effort to respond to the presence of
a character device but its response is actually incorrect and leads to the disk
being filled.  In this situation, the checker CrashSimulator used to make its
assessment needs to be expanded --- a similar situation to an insufficiently
rigorous unit test.

\preston{I am not sure about this paragraph's argument}
The opposite case is CrashSimulator reporting a failure when the application
appears to be correct.  An example of this case is the case of {\tt pr} above.
CrashSimulator reported that the application was not responding correctly but,
in reality, the application prints an error message and quits.  This case
requires some investigation.  As it turns out, {\tt pr}'s error message occurs when a
the target file is being read from rather than when it was initially examined
with {\tt stat()}.  Delaying detecting this anomalous conditions obscures its
initial appearence which hinders debugging.  Additionally the application may
fail more dramatically in environments where {\tt read()} doesn't respond in
this fashion.
