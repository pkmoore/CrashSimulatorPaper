...
\section{CrashSimulator Approach Details}
    ...
    Trace analysis -> read netcheck and checkapi papers
        NetCheck -> Has model of correct network system call order
                    Looks at order of system calls and uses heuristic algorithm and network model to determine whether
                    a particular ordering is feasible or not
                        Model encodes falsehoods commonly believed by developers
                        Prioritizes system call according to correct ordering
                        Accept, Reject, or Permanently Reject the system call

    Fault Injection
        Using ptrace to change values of system calls
        Using ptrace to buffer and change the ordering of UDP packets
        How do we determine what faults to inject specifically. E.g.\ do we make a syscall return -5 or -4 or whatever?

    Reporting
        Netcheck diagnosis engine -> How does this tie into CrashSimulator
