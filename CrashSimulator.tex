...
\section{CrashSimulator Approach Details}
    ...
    \textbf{Return Value Modification:} One way CrashSimulator can inject faults into running application is to modify
    the return values of interesting system calls identified in the previous trace analysis step. First, the
    CrashSimulator parent application is launched and configure
    ...
    Reporting
        NetCheck diagnosis engine -> How does this tie into CrashSimulator
            Based on a set of rules. If simulation state matches a rule, the associated diagnosis is emitted.
