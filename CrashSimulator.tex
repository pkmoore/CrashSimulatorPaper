...
% no \IEEEPARstart
    ...
    to the inability of the developer to replicate all possible application environments.

    The standard approach for identifying environment-induced faults at this time is to perform a beta test by deploying the
    application to a subset of users and gathering feedback related to faults via automated crash reporting or manual bug
    reports. This this group of users starts small and gradually increases over many deployments as the developer eliminates
    the most common faults and feels comfortable expanding the user base. Eventually, the application is deemed sufficiently
    free of faults and a initial production version can be published to the full user base.

    This approach proves problematic. Deployment for testing is expensive in terms of time and cost. A
    deployment may include the following steps:
    \begin{enumerate}
        \item{} A subset of users must be selected
        \item{} The users must receive training or documentation on the application
        \item{} The users must utilize the application for some period of time
        \item{} Feedback regarding faults must be elicited through either automated or manual means
        \item{} A set of faults must be derived and prioritized from the resultant feedback
        \item{} The faults must be corrected
    \end{enumerate}
    These steps are repeated with successively larger groups of users until the developer decides that the application
    is in an production ready state. Each iteration incurs the high costs associated with deployment to the users
    elimination of the faults identified. Furthermore, there is no guarantee that the developer will receive a sufficient
    quantity and quality of feedback required in order to render the application acceptably free of faults. This occurs
    for a variety of reasons. First, users may not exercise uncommon code paths sufficiently to identify faults present
    within them. This is particularly true with complex applications like web browsers or document editors. Second,
    users may opt not to submit bug reports or may submit bug reports that are of such poor quality that they are
    useless to the developer. Finally, and most importantly, there is no guarantee that these successive tests will
    exhaustively cover the possible environment configurations in which the application may be required to run.

    In light of these challenges, some other approach is needed to provide developers with confidence that their
    applications are sufficiently free of faults without requiring the often onerous testing deployment cycle.
    CrashSimulator is a tool designed to meet this need by providing automated identification of environment-induced
    faults through simulated deviations from the normal flow of system calls an application makes to interact with its
    environment.
