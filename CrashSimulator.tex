...
\section{CrashSimulator Approach Details}
    ...
    % Probably just all this general packet reordering Is there some way to determine from the system call trace
    % whether TCP or UDP was used
    \textbf{Reordering of UDP packets:} A second way CrashSimulator can inject faults into the running application
    is to buffer incoming UDP packets and reorder them

        aunch parent application
        run child application
        wait for recv
        buffer packet
        wait for subsequent recvs
        buffer further packet
        replay buffered packet


    %% Other than return codes, what else can we report on? NetCheck diagnosis engine -> How does this tie into
    %% CrashSimulator
    \textbf{Reporting:} CrashSimulator monitors the application under test during execution and reports on its
    behavior. Any executions of the application that result in the application exiting with an error code or crashing
    after injection of a fault are recorded. After all potential faults have been tested these failure-causing faults
    are reported to the user.

\section{Evaluation}

    \textbf{Bugs Identified}
    %% Intro paragraph
